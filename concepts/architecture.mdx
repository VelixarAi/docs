...
Velixar is a cloud-native memory platform built on AWS, designed for low-latency semantic recall at scale.
...
└──────────────┘     └──────────────────┘     └────────┬────────┘
                     ...
                     │  (Embeddings +  │   │  (Metadata, Auth, │   │  (Rate Limits,  │
                     │   Similarity)   │   │   Audit Trails)   │   │   Sessions)     │
                     └─────────────────┘   └───────────────────┘   └─────────────────┘
```
...
- **Event-driven** — Embedding, backups, and sync operations are triggered by database events, not polling.
...
5. Response returns with the memory ID
...
5. Top matches are returned with relevance scores
