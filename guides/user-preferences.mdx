...
url = "https://api.velixarai.com/memory"
...
old = requests.get("https://api.velixarai.com/memory/search",
    headers=headers,
...
if old:
    requests.delete(f"https://api.velixarai.com/memory/{old[0]['id']}", headers=headers)

requests.post("https://api.velixarai.com/memory", headers=headers, json={
